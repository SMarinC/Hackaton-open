...
- **Re-identificación por apariencia (heurística, `src/tracker.js`, `src/vision.js`):** cuando una persona queda oculta u sale de cuadro más de `maxGap` (1.5 s) pero reaparece dentro de una ventana corta (`reidWindow`, 6 s por defecto en la app), una firma de color local (promedio de color por celda del recorte, sin modelo nuevo ni envío de píxeles) puede revivir el mismo ID en lugar de crear uno nuevo, evitando inflar el conteo de personas. Siempre reinicia la permanencia y abre una visita nueva; nunca es identidad validada — dos personas vestidas parecido pueden confundirse.
- **Heurística de "estante vacío" (`src/shelf-occupancy.js`):** por zona, compara la densidad de bordes/contraste actual contra una línea base capturada manualmente ("Marcar estantería llena ahora" en la UI). Es una aproximación local sin modelo de producto — no identifica qué falta, solo si la zona luce menos activa que su propia referencia. Sin fotos reales de los estantes de PanelaStocks para entrenar un clasificador, esto es lo único honesto de construir en este incremento; el resultado se muestra siempre marcado como heurística no validada, y en `null` (nunca un cero engañoso) hasta que se captura una línea base.
...
8. **La permanencia de personas es una métrica para insights futuros, no el disparador de una acción de reposición.** Lo accionable (avisar a Slack, luego crear una tarea) debe fundarse en reconocer el estante, no en cuánto tiempo alguien estuvo cerca. **Implementado (`server/store.js`, `SHELF_ALERT_THRESHOLD`):** el evento de permanencia siempre abre/actualiza el caso en `review_required` (la métrica se conserva), pero el aviso `ask_review` a Slack solo se encola cuando el evento trae `shelf_occupancy_score` por debajo del umbral (heurística de `src/shelf-occupancy.js`, calculada en el navegador y enviada por `src/main.js`). Sin ese score — sin línea base marcada, o el estante luce igual o más lleno — el caso queda registrado con un timeline `shelf_alert_withheld` y sin mensaje en el outbox; no hay reintento automático posterior si la lectura mejora o empeora después.
